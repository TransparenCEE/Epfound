...
class FetchCommentsAdmin extends Job implements SelfHandling
{
    ...
    {
        $query = Comment::select('page_id')
                      ->selectRaw('COUNT(id) as aggregate')
                      ->groupBy('page_id')->getQuery();

        $method = method_exists($query, 'lists') ? 'lists' : 'pluck';

        return $query->{$method}('aggregate', 'page_id');
    }
    ...
    protected function getStatusCount($pageId = null)
    {
        ...
        $method = method_exists($query->getQuery(), 'lists') ? 'lists' : 'pluck';

        return $query->getQuery()->{$method}('aggregate', 'status');
    }
}
